...
def Attack(totalAttackRequests, serverNum, servers):
    D_set = []
    A_strategy = []
    V_set = []
    ...
    for i in range(len(servers)):
        ...
        if i == 0 or i == 1:
            A_strategy.append(servers[i].getCommonRequest())
        else:
            A_strategy.append(servers[i].getCommonRequest() - 50)
    print("A_MAX:", totalAttackRequests)
    print("D_set:", D_set)
    print("A_strategy:", A_strategy)
    print("V_set:", V_set)
    # print(serverNum)
    A_strategy_new = calculate_A_strategy(A_MAX=totalAttackRequests, D_set=D_set, A_strategy=A_strategy, V_set=V_set, wc=0.01, num=serverNum)

    return A_strategy_new
...
if __name__ == "__main__":
    ...
    #攻击策略
    A_max=1000
    A_strategy_new = calculate_A_strategy(A_max, D_set, A_strategy, V_set, wc, num)
    print(list(A_strategy_new))
    #防御策略
